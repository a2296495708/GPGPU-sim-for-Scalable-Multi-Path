...
__global__ void test_mpipdom_st_rt(int *data_in, int *data_out, int numElements) {
  int tid = threadIdx.x;
  if (tid < 8) {
    data_out[tid] = data_in[tid]  + 1000000;
    data_out[tid] = data_out[tid] + 1000000;
    data_out[tid] = data_out[tid] + 1000000;
  } else {
    data_out[tid] = data_in[tid]  + 100000;
    if (tid >= 16) {
      data_out[tid] = data_out[tid] + 1000;
      data_out[tid] = data_out[tid] + 1000;
      data_out[tid] = data_out[tid] + 1000;
      data_out[tid] = data_out[tid] + 1000;
      data_out[tid] = data_out[tid] + 1000;
...
int main(void) {
  ...
  printf("CUDA kernel launch with %d blocks of %d threads\n", blocksPerGrid,
         threadsPerBlock);
  test_mpipdom_st_rt<<<blocksPerGrid, threadsPerBlock>>>(d_data_in, d_data_out, numElements);
